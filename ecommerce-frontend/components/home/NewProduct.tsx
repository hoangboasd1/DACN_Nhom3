...
import { fetchWeeklyBestSellers, fetchProducts, addToWishlist, removeFromWishlist, checkWishlistItem, fetchCurrentUser } from '@/app/services/api';
...
const NewProduct = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        console.log('🔄 Fetching weekly best sellers...');
        const response = await fetchWeeklyBestSellers();
        console.log('✅ Weekly best sellers response:', response);
        
        if (response.data && Array.isArray(response.data)) {
          setProducts(response.data);
        } else {
          console.warn('⚠️ No weekly best sellers data received');
          setProducts([]);
        }
      } catch (err: any) {
        console.error('❌ Error fetching weekly best sellers:', err);
        ...
        try {
          ...
          if (fallbackResponse.data && Array.isArray(fallbackResponse.data)) {
            ...
            setProducts(fallbackResponse.data.slice(0, 5)); // Take first 5 products
          ...
          setError(`Failed to fetch products: ${err?.response?.data || err?.message || 'Unknown error'}`);
    ...
    return (
      <div className="container mx-auto px-4 py-16">
        <div className="flex justify-between items-center mb-12">
          <h2 className="text-2xl font-light text-black tracking-wider">Bán Chạy Trong Tuần</h2>
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      <div className="flex justify-between items-center mb-12">
        <h2 className="text-2xl font-light text-black tracking-wider">Sản Phẩm Mới</h2>
        <Link href="/products" className="text-black hover:text-gray-600 font-normal text-sm uppercase tracking-wide">
          Xem tất cả
        </Link>
      </div>
                ...
                <Link href={`/product/${product.id}`}>
                 <div className="space-y-1 mb-3">
                    ...
                    {product.createdAt !== undefined && (
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500 uppercase tracking-wide">Ngày mở bán:</span>
                        <span className="text-xs text-gray-600">{new Date(product.createdAt).toLocaleDateString('vi-VN')}</span>
                      </div>
                    )}
                  </div>
